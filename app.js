...
const pickImage = (images, target=160) => Array.isArray(images)&&images.length ? (images.reduce((b,im)=> Math.abs((im.width||target)-target) < Math.abs((b.width||target)-target) ? im : b)).url || images[0].url : null;
...
function albumsToNews(j){
  ...
  return arr.map(a => {
    ...
    return {
      source: "Spotify", link: a.url || null,
      headline: a.album_name || "Album", summary: artists ? `Artist: ${artists}` : "",
      date: a.release_date || "", language: "EN",
      cover: pickImage(a.images, 160), altText: `${a.album_name}${artists?(' by '+artists):''}`,
      genres, meta: { type:a.album_type||null, label:a.label||null, tracks:Number.isFinite(a.total_tracks)?a.total_tracks:null, release:a.release_date||null }
    };
  });
}
...
const fetchAgg = () => fetchJson(AGG_ENDPOINT, 10000).then(j => (Array.isArray(j)?j:(Array.isArray(j?.items)?j.items:[])).filter(isAllowed).map(x=>({...x,altText:x.headline||'News image'})));
const fetchSpotify = (mode='curated', market='IL', monthsBack=2) => {
  const url = `${SPOTIFY_RELEASES_ENDPOINT}?${new URLSearchParams({mode,market,monthsBack:String(monthsBack)})}`;
  return fetchJson(url, 15000).then(j => { if (j?.error) throw new Error(String(j.error)); return albumsToNews(j).filter(isAllowed); });
};

/* ====== GROUP & BALANCE ====== */
function groupAndBalance(items){
  const clean = items.filter(it => safeUrl(it.link) && (it.headline||it.summary) && !isNaN(dateToTs(it.date)));
  const groups = new Map(); for (const it of clean){ const k=(it.source||'Unknown').trim(); (groups.get(k)??groups.set(k,[]).get(k)).push(it); }
  const balanced=[]; for (const arr of groups.values()){ balanced.push(...arr.slice().sort((a,b)=>dateToTs(b.date)-dateToTs(a.date)).slice(0,MAX_PER_SOURCE)); }
  const seen=new Set(), out=[]; for (const it of balanced.sort((a,b)=>dateToTs(b.date)-dateToTs(a.date))){ const k=safeUrl(it.link)||it.headline; if(!k||seen.has(k)) continue; seen.add(k); out.push(it); }
  return out;
}
...
const renderMeta = (m,lang) => !m ? "" : `<ul class="meta-list">${
  ...
  .filter(Boolean).map(s=>`<li><span class="meta-label"></span>${s}</li>`).join("")
}</ul>`;
...
(async function init(){
  try{
    const only = getParam('only')==='spotify', mode=getParam('mode')||'curated';
    isSpotifyMode = only;
    showLoading();
    ...
    if (only){
      ...
      const sp = await fetchSpotify(mode);
      renderNews(sp.slice(0, mode==='extended'?100:MAX_ITEMS));
      // לחיצה על פילטר מוציאה מהמצב
